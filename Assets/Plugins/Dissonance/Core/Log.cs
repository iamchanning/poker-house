...
namespace Dissonance
{
    ...
    public class Log
    {
        ...
        private readonly TransferBuffer<LogMessage> _logs = new TransferBuffer<LogMessage>(128);
        private Thread _main;

        internal void WriteMultithreadedLogs()
        {
            if (_main == null)
        ...
        private void WriteLog(LogLevel level, string message)
        {
            ...
            {
                case LogLevel.Trace:
                    SendLogMessage(string.Format("TRACE {0}", msg), UnityEngine.Debug.Log);
                    break;

                case LogLevel.Debug:
                    SendLogMessage(string.Format("DEBUG {0}", msg), UnityEngine.Debug.Log);
                    break;

                case LogLevel.Info:
                    SendLogMessage(msg, UnityEngine.Debug.Log);
                    break;

                case LogLevel.Warn:
                    SendLogMessage(msg, UnityEngine.Debug.LogWarning);
                    break;

                case LogLevel.Error:
                    SendLogMessage(msg, UnityEngine.Debug.LogError);
                    break;
        ...
        private void SendLogMessage(string message, Action<string> log)
        {
#if NCRUNCH
